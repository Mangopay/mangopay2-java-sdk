...
if [ "$TRAVIS_REPO_SLUG" == "Mangopay/mangopay2-java-sdk" ] && [ "$TRAVIS_PULL_REQUEST" == "false" ] && [ "$TRAVIS_BRANCH" == "sonatype" ]; then
  ...
  ./gradlew uploadArchives -PnexusUsername="${SONATYPE_USERNAME}" -PnexusPassword="${SONATYPE_PASSWORD}" -Psigning.keyId=137F17AE -Psigning.password="${SIGNING_PASSWORD}" -Psigning.secretKeyRingFile=.utility/secretkey.gpg
  RETVAL=$?
